...
// تعریف نوع برای داده‌های سفارش
interface OrderItem {
    id: string;
    // اضافه کردن ویژگی‌های مرتبط با آیتم سفارش
}
...
interface BoxOrderProps {
    wich: number;
    data: OrderData; // تعیین نوع دقیق برای data
}
interface OrderItem {
...
interface OrderData {
    ...
    total_final_price_formatted: string;
    date: string;
    items: OrderItem[]; // This should be an array of OrderItem
}
    ...
    return (
        <div className='w-full bg-lightBlueGray rounded-2xl py-9 px-8'>
            <div className='flex justify-between items-center'>
                ...
                <div>
                    <p className='text-darkGray max-md:text-xs 	'>{data.total_final_price_formatted} تومان</p>
                </div>
            </div>
            <div className='mt-5 flex gap-4 justify-between items-center'>
                ...
                <Link href={`/profile/order/info/${data.id}`} className='flex text-customGray cursor-pointer'>
            ...
            <div className='w-full flex justify-end mt-3 max-md:hidden'>
                <p className='flex text-customGray mx-6 cursor-pointer'>
                    دریافت فاکتور <span className='mr-2'><Arrow /></span>
                </p>
                <Link href={`/profile/order/info/${data.id}`} className='flex text-customGray cursor-pointer'>
                    جزئیات سفارش <span className='mr-2'><Arrow /></span>
                </Link>
            </div>
        </div>
    );
