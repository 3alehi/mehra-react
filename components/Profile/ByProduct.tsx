...
const ByProduct: React.FC<ByProductProps> = ({items}) => {
    return (
       ...
       {items.line_item_type === "product"  &&(
         ...
         <img className='w-36 h-48 max-md:w-[113px] max-md:h-[113px]' src={items.media_files[0].main_link} alt="" />

<div className='mr-10 flex flex-col justify-between'>
<p className='text-customGray text-2xl max-md:text-sm'>{items.title}</p>
...
<p className='text-dark-gray text-lg max-md:text-sm'>{items.total_formatted} </p>
